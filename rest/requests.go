...
func getToken(link, user, password string) (string, error) {
	...
	if strings.Contains(link, "dev-cloud.khulnasoft.com") {
		token_url = "https://stage.api.cloudsploit.com"
		prov_url = "https://prov-dev.cloud.khulnasoft.com"
	} else {
		token_url = "https://api.cloudsploit.com"
		prov_url = "https://prov.cloud.khulnasoft.com"
	}
	...
	if respEvents.StatusCode == 200 {
		...
		data := raw["data"].(map[string]interface{})
		khulnasoftHostName = "https://" + data["ese_url"].(string)
	}
	return token, nil
}

func isKhulnasoftSaasFlow(link string) bool {
	return strings.Contains(link, "cloud.khulnasoft.com") || strings.Contains(link, "cloud-dev.khulnasoft.com")
}

func getData(link, user, password string) []byte {
	var req *http.Request
	if isKhulnasoftSaasFlow(link) {
		token, err := getToken(link, user, password)
		if err != nil {
			log.Fatalf("Can't receive a token to Khulnasoft SaaS: %v", err)
		}
		u, err := url.Parse(link)
		if err != nil {
			log.Fatalf("Can't parse a link %q: %v", link, err)
		}
		req, err = http.NewRequest("GET", khulnasoftHostName+u.Path, nil)
		if err != nil {
			log.Fatalf("Can't create a request to %q: %v", khulnasoftHostName+u.Path, err)
		}
